...
@section('page_title', 'Edit Drip Campaign')
...
<div class="max-w-2xl space-y-6">
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm flex items-center justify-between gap-3">
        <div>
            <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Edit Drip Campaign</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ $drip->name }}</p>
        </div>
        <a href="{{ route('drip.show', $drip) }}" class="text-sm text-teal-600 hover:underline">← Back to Steps</a>
    </div>

    <form action="{{ route('drip.update', $drip) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $drip->name) }}" required
            ...
            <div>
                ...
                <textarea name="description" rows="2"
                          class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none">{{ old('description', $drip->description) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Auto-enroll from Group</label>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('drip.show', $drip) }}" class="inline-flex items-center justify-center px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 text-sm font-medium hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                Cancel
            </a>
        </div>
    </form>
