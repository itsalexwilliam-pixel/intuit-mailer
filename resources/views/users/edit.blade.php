...
<div class="p-6 lg:p-8">
    <div class="max-w-3xl mx-auto">
        <div class="mb-6">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Update account details and role assignment.</p>
        </div>
        ...
        <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-6 sm:p-8">
            <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-5">
                ...
                <div class="rounded-lg border border-slate-200 dark:border-slate-800 p-4 bg-slate-50/80 dark:bg-slate-950/50">
                    <p class="text-sm font-medium text-slate-800 dark:text-slate-100">Change Password (Optional)</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Leave blank to keep the current password.</p>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-4">
                        <div>
                            ...
                            <input id="password" name="password" type="password"
                                class="mt-1.5 block w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 focus:border-indigo-500 focus:ring-indigo-500">
                            @error('password')
                                <p class="mt-1.5 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p>
                            @enderror
                        </div>
